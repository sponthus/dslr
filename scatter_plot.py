...
def scatter_plot(data: pd.DataFrame, feature_a: str, feature_b: str) -> None:
    ...
    if not isinstance(data, pd.DataFrame) or data.empty:
        raise ValueError("scatter_plot needs data (pd.DataFrame)")
    if "Hogwarts House" not in data.columns:
        raise ValueError("data needs `Hogwarts House` column")
    if feature_a not in data.columns:
        raise ValueError(f"'{feature_a}' not in data")
    if feature_b not in data.columns:
        raise ValueError(f"'{feature_b}' not in data")
